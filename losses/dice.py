...
from losses.cross_entropy_nd import CrossEntropyND, TopKLoss, WeightedCrossEntropyLoss

__all__ = ['GDiceLoss', 'GDiceLossV2', 'SSLoss', 'SoftDiceLoss', 'IoULoss', 'TverskyLoss', 'FocalTverskyLoss',
           'AsymLoss', 'DCAndCELoss', 'PenaltyGDiceLoss', 'DCAndTopKLoss', 'ExpLogLoss']
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        if y_onehot.ndim == 4:
            ...
            input_form = 'bcxyz'
        w: torch.Tensor = 1 / (einsum("%s->bc" % input_form, y_onehot).type(torch.float32) + 1e-10) ** 2
        intersection: torch.Tensor = w * einsum("%s, %s->bc" % (input_form, input_form), net_output, y_onehot)
        union: torch.Tensor = w * (einsum("%s->bc" % input_form, net_output) + einsum("%s->bc" % input_form, y_onehot))
...
class DCAndTopKLoss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum"):
        super(DCAndTopKLoss, self).__init__()
        self.aggregate = aggregate
...
class ExpLogLoss(nn.Module):
    ...
    def __init__(self, dice_kwargs, wce_kwargs, soft_dice=True, gamma=0.3):
