...
from models.stylegan2_ada_spade import misc
from models.stylegan2_ada_spade.ops import bias_act, conv2d_resample, fma, upfirdn2d
from models.stylegan2_ada_spade.spade import SPADE
...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=256, channels_last=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
        memory_format = torch.channels_last if channels_last else torch.contiguous_format
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.spade_label_nc = spade_label_nc

        if use_spade:
            self.spade = SPADE(3, out_channels if in_channels == 0 else in_channels, spade_label_nc)
    ...
    def forward(self, x, img, ws, spade_label_map=None, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.use_spade:
            misc.assert_shape(spade_label_map, [None, self.spade_label_nc, None, None])
            x = self.spade(x, spade_label_map)
            # x = x.to(dtype=dtype)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
                 ...
                 use_spade=False,  # whether to use spade to incorporate
                 spade_label_nc=2,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                   ...
                                   use_spade=use_spade, spade_label_nc=spade_label_nc, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
            setattr(self, f'b{res}', block)

    def forward(self, ws, spade_label_map=None, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            x, img = block(x, img, cur_ws, spade_label_map, **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels,
                                          use_spade=use_spade, spade_label_nc=spade_label_nc, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

    def forward(self, z, c, spade_label_map=None, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        img = self.synthesis(ws, spade_label_map, **synthesis_kwargs)
        return img
...
class MinibatchStdLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # avoid last minibatch size not elegant
        if N % G != 0:
            G = N
        F = self.num_channels
        c = C // F
