...
from models.stylegan2_ada_spade.ops import conv2d_gradfix, grid_sample_gradfix, upfirdn2d
...
augpipe_specs = {
    ...
    'rotate-mnist': dict(xflip=1, xint=1, scale=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1),
    'bg': dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1),
    'bgfnc': dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, imgfilter=1, noise=1, cutout=1),
    'bgc': dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1,
                hue=1, saturation=1),
    'bgcn': dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1,
                 hue=1, saturation=1, noise=1, noise_std=0.02),
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.hue > 0 and num_channels == 3:
        ...
        if self.saturation > 0 and num_channels == 3:
        ...
        if C is not I_4:
            ...
            elif num_channels == 2:  # 1+1
                C = C[:, :3, :].mean(dim=1, keepdims=True)
                images_1 = images[:, :1] * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                images_2 = images[:, 1:] * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                images = torch.cat([images_1, images_2], dim=1)
            elif num_channels == 4:  # 3 + 1
                images_1 = C[:, :3, :3] @ images[:, :3] + C[:, :3, 3:]
                C = C[:, :3, :].mean(dim=1, keepdims=True)
                images_2 = images[:, 3:] * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                images = torch.cat([images_1, images_2], dim=1)
            else:
                raise ValueError('Image must be RGB (3 channels) or L (1 channel) or L+L (2 channels) '
                                 'or RGB+L (4 channels)')
            images = images.reshape([batch_size, num_channels, height, width])
